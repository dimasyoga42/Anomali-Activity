...
export const useAnomaliStore = create((set) => ({
  ...
  fetchMember: async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("userlogin")
        .select("id, name, ign, role, image_url");
      if (error) throw error;
  ...
  fetchBanner: async () => {
    set({ loading: true, error: null });
    try {
      const { dataBanner, error } = await supabase
        .from("banner")
        .select("title, link")
        .limit(3);
  ...
  deleteMember: async (id) => {
    ...
    try {
      const { error } = await supabase.from("userlogin").delete().eq("id", id);
  ...
  fetchBlog: async () => {
    ...
    try {
      ...
      const authorIds = [
        ...new Set(blogData.map((b) => Number(b.author_id)).filter(Boolean)),
      ];
      ...
      const blogsWithAuthors = blogData.map((b) => {
        const author = authorsData?.find(
          (a) => Number(a.id) === Number(b.author_id),
        );
        return {
          ...b,
